...
// Source: gameslate/lib/schedule-conflicts.ts at 2810c62 (2026-09-04).
...
export interface ConflictGame {
  ...
  home_team_id: string;
  division?: string;
  /** This game's own length in minutes (an 8U game at 60, a 14U game at
   *  105). Falls back to the league-wide gameMinutes when absent. */
  minutes?: number;
}
...
export function findConflicts(
  ...
  const lenOf = (g: ConflictGame) => (g.minutes != null && g.minutes > 0 ? Math.floor(g.minutes) : gameMinutes);
  ...
  for (const [, bucket] of byDateField) {
    ...
    for (let i = 0; i < bucket.length; i++) {
      for (let j = i + 1; j < bucket.length; j++) {
        ...
        if (!timesOverlap(a.g.time, b.g.time, lenOf(a.g), lenOf(b.g))) continue;
